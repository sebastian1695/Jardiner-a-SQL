...
select*from oficina;
alter table oficina  drop linea_direccion2;
alter table oficina  drop linea_direccion1;
alter table oficina  drop codigo_postal;
alter table oficina  drop region;
select*from oficina;
...
alter table oficina  drop codigo_postal;
alter table oficina  drop region;
alter table oficina drop ciudad;
alter table oficina drop codigo_oficina
...
alter table empleado drop codigo_empleado;
alter table empleado drop extension;
alter table empleado drop codigo_oficina;
...
select* from pago;
select date_format('2008-11-10', '%y');

alter table pago drop total;
alter table pago drop id_transaccion;
alter table pago drop forma_pago;
select codigo_cliente, count(codigo_cliente) from pago
group by codigo_cliente
having count(codigo_cliente) >1; 

select*from pago
where fecha_pago between '2008-01-01' and '2008-12-31';
delete t1 from pago t1
inner join pago t2
where t1.codigo_cliente < t2.codigo_cliente and
t1.total=t2.total;
...
QUEDO PENDIENTE DE ESTE PUNTO, NO SÉ PORQUÉ NO ME DEJÓ BORRAR LOS DUPLICADOS
...
USA TIENE 4 CLIENTES

select count(*)
from cliente
where pais='Spain'; 

Spain tiene 27 clientes

select count(*)
from cliente
where pais='France';

France tiene 2 clientes

select count(*)
from cliente
where pais='AUSTRALIA';

Australia tiene 2 clientes 

select count(*)
from cliente
where pais='United Kingdom'; 

United Kingdom tiene un cliente 

¿Cuál fue el pago medio en 2009?

select avg(total) 'promedio de pago en 2009' from pago
...
select count(*) 'Cantidad de pedidos rechazados'
from pedido
where estado= 'Rechazado';

select count(*) 'Cantidad de pedidos pendientes'
from pedido
where estado= 'Pendiente';

select count(*) 'Cantidad de pedidos entregados'
from pedido
...
select max(precio_venta) 'producto más caro', min(precio_venta) 'producto más económico' from producto;
...
MAX(limite_credito) 'Límite de crédito más alto' FROM cliente
GROUP BY nombre_cliente
HAVING nombre_cliente ='Tendo Garden';

Devuelve el nombre del producto que tenga el precio de venta más caro

SELECT*FROM PRODUCTO;
SELECT NOMBRE, 
MAX(PRECIO_VENTA) 'PRODUCTO MÁS CARO' FROM PRODUCTO
GROUP BY NOMBRE
HAVING NOMBRE='Trachycarpus Fortunei';

Devuelve el nombre del producto del que se han vendido más unidades.(Tenga en cuenta que tendrá que calcular cuál es el número total de unidades que se han vendido de cada producto a partir de los datos de la tabla detalle pedido)
SELECT*FROM detalle_pedido;

select codigo_producto,
sum(cantidad) from detalle_pedido
group by codigo_producto
having codigo_producto='FR-67';

Del producto FR-67, la cantidad es de 285

Los clientes cuyo límite de crédito sea mayor que los pagos que haya realizado. (Sin utilizar INNER JOIN).

select*from pago;
select P.*, C.* From cliente C, pago P
where C.codigo_cliente = P.codigo_cliente;

select codigo_cliente, 
total from cliente C, pago P; 
limite_credito;
...
select*from cliente;
select codigo_cliente,
limite_credito from cliente
having limite_credito >
